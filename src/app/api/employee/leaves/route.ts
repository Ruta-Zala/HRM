...
import {
  ...
  hasLeaveBucketStorage,
  importLeaveBucketCsv,
  readLeaveBucketRows,
  toLeaveBucketStorageRef,
...
export const POST = withActiveSession(async (req, user) => {
  ...
  try {
    ...
    if (!hasLeaveBucketStorage(employee)) {
      return NextResponse.json(
        {
          success: false,
          message: "Employee leave storage not found",
        },
        { status: 404 },
      );
    }

    const leaveRef = toLeaveBucketStorageRef(employee!);

    if (typeof body.csv === "string" && body.csv.trim()) {
      await importLeaveBucketCsv(leaveRef, body.csv);
    ...
    if (leaveType === "birthday") {
      ...
      const rows = await readLeaveBucketRows(leaveRef);
      ...
      await addGroupedLeaveDatesToBucket(leaveRef, groups, "full", "");
      ...
      try {
        await notifyLeaveSubmitted({
          ...
          leaveType: "birthday",
          dateRange,
          applicationId: `${employee!.employeeId}:birthday:${leaveDateIso}:${requestId}`,
        });
      } catch (notifyError) {
    ...
    await addGroupedLeaveDatesToBucket(leaveRef, groups, duration, reason);
    ...
    try {
      await notifyLeaveSubmitted({
        ...
        dateRange,
        reason,
        applicationId: `${employee!.employeeId}:${fromDate}:${toDate}:${leaveType}:${requestId}`,
      });
    } catch (notifyError) {
      console.error("Leave submit notification error:", notifyError);
    }
